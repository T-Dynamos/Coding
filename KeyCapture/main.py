...
class MainView(MDApp):
    ...
    def on_keyboard(self, *largs):
        if largs[-2] is not None and largs[-2] in list(
            ...
            print(largs)
            self.root.ids.key_label.text = (
                str(
                    largs[-1][-1] if largs[-1][-1] != "capslock" else largs[-1][0]
                ).capitalize()
                if len(largs[-1]) != 0 and len(largs[-1]) >= 1
                else "Capslock"
            )
